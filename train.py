...
from utils import get_stream_logger, simulation, selection, crossover, mutation
...
def main(args):
    ...
    logger.info(f"num_populations == {num_populations}")
    print(f"num_populations == {num_populations}")
    logger.info(f"p_mutation == {p_mutation}")
    print(f"p_mutation == {p_mutation}")
    logger.info(f"num_select == {num_select}")
    print(f"num_select == {num_select}")
    logger.info(f"max_generations == {max_generations}")
    print(f"max_generations == {max_generations}")
    logger.info(f"mode == {selection_method}")
    print(f"mode == {selection_method}")
    logger.info(f"num_validations == {num_validations}")
    print(f"num_validations == {num_validations}")

    # Initialize
    populations = [Model() for i in range(num_populations)]
    fitnesses = simulation(
        populations, generation=0, n_valid=num_validations, logger=logger
    )
    ...
    populations = [
        crossover(selected_model[0], selected_model[1], alpha=0.5)
        for i in range(num_populations - 2)
    ] + [deepcopy(selected_model[0]), deepcopy(selected_model[1])]
    del selected_model
    ...
    for i in tqdm(range(max_generations), desc=f"Generation"):
        fitnesses = simulation(
            populations, generation=i + 1, n_valid=num_validations, logger=logger
        )
        selected_idx = selection(fitnesses, num_select, method=selection_method)
        ...
        if i + 1 in save_indexes:
            torch.save(
                selected_model[0].state_dict(), f"{savedir}/generation{i + 1}.pt"
            )
    ...
    logger.info(f"best fit : {best_fitness}")
